 ...
 *
 * @param sampleRate of the samples
 * @param symbolRate of the signal
 */
void create_costasLoop(double sampleRate, double symbolRate) {
    mSampleRate = sampleRate;

    mCurrentVector = 0.0;
    mMaximumLoopFrequency = 1.0; //TAU * (symbolRate / 2.0) / sampleRate;
    mPLLBandwidth = TAU / BW_200;
...
static void updateLoopBandwidth() {
    double denom = ((1. + (2. * mDamping * mPLLBandwidth)) + (mPLLBandwidth * mPLLBandwidth));

    mAlphaGain = (4. * mDamping * mPLLBandwidth) / denom;
    mBetaGain = (4. * mPLLBandwidth * mPLLBandwidth) / denom;
}
...
void setPLLBandwidth(PLLBandwidth pllBandwidth) {
    double val = TAU / (double) pllBandwidth;

    if (mPLLBandwidth != val) {
        mPLLBandwidth = val;
...
void costas_adjust(double phaseError) {
    ...
    mFrequencyError = (mSampleRate / TAU * mLoopFrequency);
}
